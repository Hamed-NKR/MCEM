function h = PA_VS_NPP_v1(parsdata, sigma_g)
...
if (~exist('sigma_g', 'var'))  || isempty(sigma_g)
    sigma_g = 1; % aggs are internally monodisperse
end
...
a01 = 0.3757 * n0 + 0.4098 * n0.^0.7689;
p{1} = plot(n0, a01 ./ n0, 'Color', [0.4940 0.1840 0.5560], 'LineStyle', '-.',...
    'LineWidth', 2.5);
legtxt{1} = 'Meakin et al. (1989)';
hold on

% plot Dastanpour & Rogak (2014) correlation
a02 = (0.94 + 0.03 * sigma_g^4.8) * n0.^0.46;
p{2} = plot(n0, a02 ./ n0, 'Color', [0 0.4470 0.7410], 'LineStyle', ':',...
    'LineWidth', 2.5);
legtxt{2} = 'Dastanpour & Rogak (2016)';

% plot temporal aggregate area vs. number data
for i = 1 : n_dat
    p{i + 2} = scatter(parsdata(i).npp, ((parsdata(i).da ./...
        parsdata(i).dpp_g(:,1)).^2 * pi / 4) ./ parsdata(i).npp,...
        ms(i), mc(i,:), mt{i}, 'LineWidth', 0.1);
    
    legtxt{i + 2} = strcat('\itn_{agg}\rm/\itn_{agg_0}\rm =',...
        {' '}, num2str(t_id(i), '%.2f'));
end
