function pp = MANUPP_v2(fname_pp, da, fad_pp, n_hyb, opts)
...
if ismember(opts_visual, {'ON', 'On', 'on'})
    ...
    p{1} = scatter(da(ii{1,1}), pp.mu_d(ii{1,1}), 60, 's',...
        'MarkerEdgeColor', [0.8500 0.3250 0.0980]);
    legtxt{1} = 'Argonaut burner - Uniform';
    
    p{2} = scatter(da(ii{1,2}), pp.mu_d(ii{1,2}), 60, 's',...
        'MarkerFaceColor', [0.8500 0.3250 0.0980], 'MarkerEdgeColor', [0.8500 0.3250 0.0980]);
    legtxt{2} = 'Argonaut burner - Hybrid';
    
    p{3} = scatter(da(ii{2,1}), pp.mu_d(ii{2,1}), 40, '^',...
        'MarkerEdgeColor', [0.9290 0.6940 0.1250]);
    legtxt{3} = 'Flare - Uniform';
    
    p{4} = scatter(da(ii{2,2}), pp.mu_d(ii{2,2}), 40, '^',...
        'MarkerFaceColor', [0.9290 0.6940 0.1250], 'MarkerEdgeColor', [0.9290 0.6940 0.1250]);
    ...
    if ismember(opts_eb, {'ON', 'On', 'on'})
        ...
        eb{1} = errorbar(da(ii{1,1}), pp.mu_d(ii{1,1}),...
            e_n(ii{1,1}), e_p(ii{1,1}), 'Marker', 'none',...
            'LineStyle', 'none');
        eb{1}.Color = [0.8500 0.3250 0.0980];
        
        eb{2} = errorbar(da(ii{1,2}), pp.mu_d(ii{1,2}),...
            e_n(ii{1,2}), e_p(ii{1,2}), 'Marker', 'none',...
            'LineStyle', 'none');
        eb{2}.Color = [0.8500 0.3250 0.0980];

        eb{3} = errorbar(da(ii{2,1}), pp.mu_d(ii{2,1}),...
            e_n(ii{2,1}), e_p(ii{2,1}), 'Marker', 'none',...
            'LineStyle', 'none');
        eb{3}.Color = [0.9290 0.6940 0.1250];

        eb{4} = errorbar(da(ii{2,2}), pp.mu_d(ii{2,2}),...
            e_n(ii{2,2}), e_p(ii{2,2}), 'Marker', 'none',...
            'LineStyle', 'none');
        eb{4}.Color = [0.9290 0.6940 0.1250];
    end
