...
j_max = 1e4 * ones(mixlev,1); % Marching index limit
...
for i = 1 : mixlev    
    ...
    pars = PAR.INIT_LOC(pars, params_ud); % Randomize initial locations
    ...
    while (j <= j_max(i)) && (all(cat(1, pars.n) < (params_ud.Value(5) / 3)))
...
pars = PAR.INIT_LOC(pars, params_ud); % Assign random locations to aggregates
...
% Stage 2 DLCA
while (k <= k_max) && (length(cat(1, pars.n)) > (n_agg0 / 3))
    [pars, delt] = TRANSP.MARCH(pars, fl, params_const); % Solve for transport
