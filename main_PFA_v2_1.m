...
%     'da', cell(n_kk + 1, 1)); % Placeholder for hybrid aggregates data
% 
% parsdata(1).da = da1;
...
%         parsdata(kkk + 1).da = 2 * sqrt(PAR.PROJECTION(pars, [], 1e4, 20) / pi); % Get projected area size
%         
...
%     parsdata(kkk + 1).dpp = pars.dpp;
%     parsdata(kkk + 1).dpp_g = pars.dpp_g;
%     parsdata(kkk + 1).da = 2 * sqrt(PAR.PROJECTION(pars, [], 1e4, 20) / pi);
%     
%     kkk = kkk + 1;
% end
% 
