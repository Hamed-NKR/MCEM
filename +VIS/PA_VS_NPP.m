...
n0 = log(logspace(exp(0.1), exp(10), 1e4));
a0 = 0.3757 * n0 + 0.4098 * n0.^0.7689;
p{1} = plot(n0, a0 ./ n0, 'Color', [0.5 0.5 0.5], 'LineStyle', '-.',...
    'LineWidth', 2.5);
legtxt{1} = 'Meakin et al., 1989';

% plot temporal aggregate area vs. number data
for i = 1 : 6
    p{i + 1} = scatter(parsdata(i).npp, (pi * parsdata(i).da.^2 / 4) ./ parsdata(i).npp,...
...
xlabel('{\itPA_{agg}}/{\itn_{pp}} [nm^2]', 'FontName', 'Calibri', 'FontSize', 16)
set(gca, 'XScale', 'log')
