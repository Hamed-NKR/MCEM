function [r_cm, rad_vol] = EQUIV(pp, n_pp)
...
% Outputs:
    ...
    % rad_vol: Aggregate equivalent volumetric diameter
...
r_cm = zeros(n_agg,3);
rad_vol = zeros(n_agg,1);
for i = 1 : n_agg
    r_cm(i,:) = sum(r_temp{i}) ./ sum(vol_pp{i}); % Calculating the...
        % ...center of mass
    rad_vol(i) = nthroot(6 .* sum(vol_pp{i}) ./ pi, 3); % Calculating...
